...
async def cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "m_prof":
        ...
        txt = (
            ...
            f"🪙 *Vault Coins:* `{p['coins']} Coins`\n"
            f"⚔️ *Matches Played:* `{p['matches']}`\n"
            f"👑 *Victories:* `{p['wins']}`\n"
            f"📈 *Win Rate:* `{wr}%`\n"
            f"🔥 *Current Streak:* `{p['streak']} in a row`"
        )
        await q.edit_message_text(txt, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Main Controller", callback_data="main")]]), parse_mode="Markdown")
        return
    ...
    if d == "m_ld":
        ...
        lines = []
        for i, (name, sc, w, c) in enumerate(rows, 1):
            badge = "👑" if i == 1 else ("🥈" if i == 2 else ("🥉" if i == 3 else f"{i}."))
            lines.append(f"{badge} *{name}* — `{sc} PTS` ({w} wins)")
        t = "🏆 *HALL OF FAME (TOP PLAYERS)*\n\n" + ("\n".join(lines) if lines else "No games played yet!")
        await q.edit_message_text(t, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Menu", callback_data="main")]]), parse_mode="Markdown")
        return

    # --- Tic Tac Toe ---
    if d == "m_ttt":
        kb = [
            [InlineKeyboardButton("🤖 Play vs AI", callback_data="t_ai")],
            [InlineKeyboardButton("👥 1v1 PvP Challenge", callback_data="t_pvp")],
            [InlineKeyboardButton("🔙 Menu", callback_data="main")]
        ]
        await q.edit_message_text("❌⭕ *TIC TAC TOE ARENA*\nGame Mode select karo:", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown"); return
    ...
    if d == "t_pvp":
        ...
        kb = [[InlineKeyboardButton("⚔️ Join Challenge", callback_data=f"tj_{gid}")]]
        await q.edit_message_text(f"⚔️ *1v1 LOBBY*\nHost: `{u.first_name}` (❌)\nWaiting for friend to join...", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown"); return
    ...
    if d.startswith("tm_"):
        ...
        if g['mode'] == 'pvp' and u.id != (g['p1'] if g['turn'] == 'X' else g['p2']):
        ...
        if w:
            ...
            win_txt = "🤝 Match Draw!" if w == "Draw" else f"🎉 Winner: *{g['p1_n'] if w=='X' else (g['p2_n'] if g['mode']=='pvp' else 'AI')}* (+15 PTS)!"
    ...
    if d == "m_g":
        ...
        guess[cid] = {'a': item['answer'], 'd': item['display'], 'cat': c}
        await q.edit_message_text(f"🎯 *GUESS MASTER ({c.upper()}):*\n`{item['clue']}`\n\n💬 Direct chat me answer likho!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Menu", callback_data="main")]]), parse_mode="Markdown"); return

    # --- Word Knockout ---
    if d == "m_w":
        wb[cid] = {'st': 'lob', 'pls': {u.id: u.first_name}, 'r': 1, 'safe': set(), 'used': set()}
        kb = [[InlineKeyboardButton("🙋‍♂️ Join Battle", callback_data="wj"), InlineKeyboardButton("🚀 Start (Host)", callback_data="ws")], [InlineKeyboardButton("🔙 Menu", callback_data="main")]]
        await q.edit_message_text(f"🔤 *WORD KNOCKOUT LOBBY*\nJoined: `{u.first_name}`\n\nDosto ko bolo Join dabayein!", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown"); return

    if d == "wj":
        if cid in wb and wb[cid]['st'] == 'lob':
            wb[cid]['pls'][u.id] = u.first_name
            await q.answer("Battle Joined!")
        return
    ...
    if d == "m_b":
        ...
        await q.edit_message_text("🔢 *ROYAL BINGO ARENA*\n1 se 25 ka card banao aur draw dabao!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎲 Draw Number", callback_data="b_d")], [InlineKeyboardButton("🔙 Menu", callback_data="main")]]), parse_mode="Markdown"); return

    if d == "b_d":
        if not bingo.get(cid): await q.answer("Saare numbers draw ho gaye!"); return
        n = bingo[cid].pop()
        await q.edit_message_text(f"📢 *CALL: [ {n} ]*\n{BINGO_CALLS.get(n, '')}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎲 Draw Next", callback_data="b_d")], [InlineKeyboardButton("🔙 Menu", callback_data="main")]]), parse_mode="Markdown"); return
    ...
    if d == "m_q":
        ...
        await q.edit_message_text(f"🧠 *HIGH-IQ CHALLENGE:*\n`{item['q']}`", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown"); return

    if d.startswith("qa_"):
        if int(d.split("_")[1]) == qz.get(cid):
            add_stats(u.id, u.first_name, score_add=15, coins_add=10, is_win=True)
            t = f"🎉 *BRILLIANT {u.first_name}!* Sahi jawab (+15 PTS, +10 Coins) ⭐"
        else:
            add_stats(u.id, u.first_name, score_add=0, coins_add=0, is_win=False)
            t = "❌ *Galat!* Agli baar dhyan se socho."
        await q.edit_message_text(t, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("▶️ Next Question", callback_data="m_q"), InlineKeyboardButton("🔙 Menu", callback_data="main")]]), parse_mode="Markdown"); return

    # --- Mines Roulette ---
    if d == "m_m":
        gid = f"m_{u.id}_{random.randint(100,999)}"
        mines[gid] = {'b': ["?"]*9, 'm': set(random.sample(range(9), 2)), 'd': 0, 'p': u.id, 'p_n': u.first_name}
        await q.edit_message_text("💎 *MINES ROULETTE*\nDiamonds dhoondo, 💣 Bomb se bacho!", reply_markup=render_mines(mines[gid], gid), parse_mode="Markdown"); return
    ...
    if d.startswith("mc_"):
        ...
        if idx in g['m']:
            add_stats(u.id, u.first_name, is_win=False)
            del mines[gid]
        ...
        await q.edit_message_text(f"💎 Diamonds Found: `{g['d']}`", reply_markup=render_mines(g, gid), parse_mode="Markdown"); return
    ...
    if d.startswith("mcash_"):
        ...
        if g:
            ...
            await q.edit_message_text(f"💰 *CASHOUT!* `{u.first_name}` bagged *+{pts} PTS & Coins*!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Menu", callback_data="main")]]), parse_mode="Markdown"); return
    ...
    if d == "m_mem":
        ...
        await q.edit_message_text("❓ Sequence wale emojis chat me type karo!")
        return

# --- 5. Group Message & Chat Input Listener ---
async def msg_h(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cid in guess and (t == guess[cid]['a'] or guess[cid]['a'] in t):
        add_stats(u.id, u.first_name, score_add=15, coins_add=10, is_win=True)
        await update.message.reply_text(f"🎉 Correct: *{guess[cid]['d']}* (+15 PTS, +10 Coins) ⭐")
        del guess[cid]

    # Memory Matrix
    if cid in mem and "".join(mem[cid]) == t.replace(" ", ""):
        add_stats(u.id, u.first_name, score_add=20, coins_add=15, is_win=True)
        await update.message.reply_text(f"🧠 Laser Focus! (+20 PTS, +15 Coins)")
        del mem[cid]
